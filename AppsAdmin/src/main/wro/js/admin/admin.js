        ...
        buttons: {
        	"delete": function() {            	
				...
				jQuery.ajax({  
					...
					type: 'POST',  
					url: '<?php echo $this->restPrefix ?>/mobileApplications/d/' + mainID, 
...
function openAndPopulateForm(index) {
	...
	jQuery("#mobileApplicationId").val(row.mobileapps_application_id)
	
	jQuery("#restName").val(row.rest_name);
		
	jQuery("#name").val(row.name);
	jQuery("#description").val(row.description);
		
	var supportedLocales = jQuery("#h_supported_locales");
	supportedLocales.val(row.supported_locales);
	...
	jQuery("#main_locale").val(row.main_locale);
...
function linkFormatter(cellvalue, options, rowObject) {	
	return '<a href="#" onclick="openAndPopulateForm(' + (options.rowId - 1) + ')" style="display: block">' + cellvalue + ' (' + rowObject.restName + ')</a>';
}
